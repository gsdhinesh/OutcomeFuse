...
scope: 'The OutcomeFuse MVP in full — governed runtime, conditional mechanisms, operating modes, outward ports, evidence pipeline and submission artifact (F1–F13, F15–F19, FR1–FR121, NFR1–NFR12). F14 is superseded by F17–F19.'
status: final
created: '2026-09-07'
updated: '2026-09-14'
binds: [F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12, F13, F15, F16, F17, F18, F19]
sources:
...
- **Binds:** F2, F4, F13, FR7, FR8, FR9, FR10, FR21, FR65, FR108, FR122
- **Prevents:** Remote code execution driven by a data file, in a product whose position is that platform teams hand contracts to agent owners — and a contract hash that attests to a verifier whose behaviour can change underneath it. **Also prevents a write tool reaching the runtime with no human gate on it**, by refusing the contract at load rather than trusting the author (FR122).
...
- **Rule:** Approval is obtained through an `ApprovalPort`. Two implementations exist: a **scripted decider driven by the case definition** (approve · deny · never respond), which is **the harness's** and the only one any reported benchmark run uses; and the **boundary adapter of AD-24**, through which a human answers. A contract is never bound to one channel.

  **The driver owns the clock.** An adapter that could hold a pause open could extend `approval_timeout`, moving a contract term into whoever wrote the client.
...
- **Binds:** F17, F18, F19, F13, F16, FR72–FR77, FR97
- **Prevents:** FR77's "no path to influence execution" degrading into a discipline, and a protected submission depending on a cut-first component.
  **Rule:** The library contains **no rendering code and no viewer process**. It publishes decisions (AD-22) and asks for approvals (AD-24); every experience — tree view, chat, IDE panel, static comparison page — is a **client built outside the distribution**, depended on by nothing. A client never reaches the evidence store. **It may subscribe to the published stream while a run executes (AD-22); it may read the decision log only once that run is sealed.** Those are different acts on different artifacts — the stream is offered to it, the log is not.

  The **static HTML generator remains a valid client** and is still how the recorded side-by-side comparison is produced — one self-contained file per comparison, embedding the replay timeline, ledger state at each decision, the rendered proof card and enough record to satisfy FR76 offline. It computes no proof figures (FR97 owns those), reads the **decision log and proof card only, never the evidence store**, and enables autoescaping **explicitly** at template-environment construction — it is opt-in, not the default — because record content is untrusted input.

  > **Amended 2026-09-14, ID and Prevents retained.** The original rule was "generated, not served", chosen because a generated file made FR77 structural — no live process existed that could touch execution. FR113 now requires a live process that deliberately can. The grounds are void; the intent is not, and it survives in a stronger form. Read-only is now enforced by the **package boundary** rather than by declining to write a server: there is no rendering code in the library to disable, so there is nothing a future contributor can quietly wire back into the execution path.
...
### AD-22 — Publication is one-way and off the critical path

- **Binds:** F17, AD-2, AD-3, AD-16, AD-20, FR110, FR111, FR117, NFR1
- **Prevents:** A subscriber becoming able to slow, stall or reorder governed execution; a client tailing the record store behind the writer; and publication cost landing in the net figure, which would make the measurement depend on whether anyone was watching.
- **Rule:** **The log is appended first, then the decision is enqueued for publication.** A crash between the two costs a subscriber its view of a decision the log still holds; the reverse order would let a client observe a decision the record never contains, and AD-2 makes the log the system of record. Publication is never what makes a decision real.

  The driver **enqueues and returns**. Each subscriber holds a **bounded buffer**; on overflow the subscriber is **dropped** — never blocked, never allowed to apply back-pressure. **No subscriber is required**, and a run with none behaves identically to a run with ten.

  **Every published decision carries a monotonic per-run sequence number**, and a dropped subscriber's channel is closed with a terminal truncation signal. A client therefore detects its own gaps without being told, and re-reads the sealed log afterwards if it cares. **Nothing about a subscriber is written to the decision log:** AD-3's lane carries decisions, AD-8 owns a closed event schema, and a viewer's health is not a fact about the run.

  A client **never reads the record store for a run in progress** — not the log, not the writer lock. AD-16 serialises writers under WAL and AD-2 makes the log the system of record; a reader behind the writer would be depending on internals this spine is free to change, and would couple every future experience to the storage engine.

  **Publication cost is not governor overhead** (FR15). The stream is evidence *for a human*; the log is the evidence *of record* and is written synchronously regardless. The asymmetry is deliberate: **the log is durable and the stream is lossy by design.** The drop is nonetheless recorded, because a client that silently missed decisions renders a clean run that never happened — a worse artifact than no client.

  Placed in **AD-20's fail-open family**: the port raising, or having no listener, never reaches the run.

  > Added latency is the binding threshold in this system, not overhead share — measured at ~3.5ms p50 per decision with ~93% of it the durable appends. A synchronous publish on the decision path was the cheapest available way to destroy it.

### AD-23 — Wire types are record types

- **Binds:** F17, F18, AD-5, AD-8, AD-21, FR112, FR121, FR56, NFR5
- **Prevents:** AD-5's redaction-at-the-port degrading into a review discipline the moment a process boundary exists — and every future field having to be re-checked for whether it may cross.
- **Rule:** The stream and approval ports may serialize **only types already defined by the record schema (AD-8) and the approval port**. **No wire-only DTO, and no field the record does not already hold.** Tool arguments and tool results are therefore unpublishable by construction, not by review.

  This is already true of what exists and the rule exists to keep it true: `ApprovalRequest` is `run_id · step_id · tool · clause · timeout_seconds` with `extra="forbid"`, and a decision event carries `tool` and `canonical_key` and never the arguments behind it.

  **The constraint is on content, not envelope.** A transport may carry what it needs to be a transport — a sequence number (AD-22), a correlation id, an approval authorisation (AD-24) — provided each is **generated at the boundary, carries no run content, and is never persisted as record**. What may never cross is a *content* field the record does not hold. Without this line AD-24's bound authorisation would be unimplementable, since the record has no reason to carry a nonce and `extra="forbid"` would reject one.

  **Publication is egress, so AD-21's refusal extends to it**: the port refuses to publish for any run whose `data_class` is not `synthetic`. AD-21 was written when the store was the only way out; a boundary is a second way out, and a subscriber the library cannot see will persist whatever it receives.

  > Redaction-by-review must be got right on every field anyone ever adds. Redaction-by-construction is got right once, and a violation is a type error rather than a judgement call.

### AD-24 — The approval boundary is same-host, and its authorisation is bound

- **Binds:** F18, AD-12, AD-20, FR113, FR114, FR118, FR119, FR120, FR89, FR95
- **Prevents:** A captured approval authorising a *later* gated call; an ordinary client lifecycle event resolving a governance gate; and an unverified approver name being read as governance evidence.
- **Rule:** The boundary is **loopback, same-host, with a local token**. A networked deployment is a later adapter behind the same port shape and is not MVP capability.

  Every pending approval carries an authorisation **bound to its `run_id` and `step_id`, single-use, and dead at pause close**. Reused, expired or mismatched authorisations are refused and recorded as refused — neither approval nor denial. **Single-use is validated against the run's recorded state, never against adapter memory**: an adapter holding its nonce set in process would accept a replayed authorisation after a restart, so the check is rebuilt from the record on startup.

  **The adapter raising is `channel-unavailable`**, resolved fail-closed per the placement below — never a timeout, never approval.

  **No transport or client event is ever a decision.** A close, refresh, navigation, dropped connection, reconnect-into-fresh-state or crash resolves nothing; an unanswered pause resolves only through FR95's `on_timeout`. AD-12's two failure states carry the boundary unchanged: *accepted but silent* is `no-response`; *no client reachable, none configured, or channel lost mid-pause* is `channel-unavailable`.

  **Approver identity is read off the connection, never off the payload.** The operating system's peer credential — the client's SID via named-pipe impersonation on Windows, `SO_PEERCRED` on a Unix domain socket — is the identity; a payload-asserted identity is **refused, not recorded**. **Authorization is a separate question the MVP does not answer:** no contract declares which principals may satisfy an approval clause, so every interactive run carries `authorization-unchecked` and no claim of proper authorisation rests on the record.

  Placed in **AD-20's fail-closed family**: no reachable client means the gated call is not made.

  > Same-host earns its place twice. It sizes the authorisation obligation, and it hands verified identity over for free — the operating system already knows which account opened the connection, no identity provider or token validation is involved, and nothing the client sends can change the answer. **The transport is consequently constrained by this AD, not free:** a named pipe or Unix socket carries peer identity, loopback TCP does not, so an HTTP approval endpoint would have to resolve the owning process through the connection table — fragile, racy against port reuse, and wrong silently. The stream port has no such constraint, which is a second and independent reason these are two ports rather than one.

  > What remains missing is entitlement, and it is labeled rather than implied. A valid user is not a valid approver; conflating them makes a gate into theatre with an audit trail. On one development host the run operator and the approver are also the same account — the mechanism is proven, separation of duties is not.
...
| Dependency direction | One way, always: `clients/` → `src/outcomefuse/`. **Nothing under `src/` imports from `clients/`**, and the packaged distribution does not contain them (AD-17). A client depends on published types only; it is never granted a back door the ports do not already offer every other client. |
...
outcomefuse/
  ...
  src/outcomefuse/
    ...
    adapters/
      ...
      approval/
        scripted/           # AD-12: the harness's decider - every reported run uses this. EXISTS.
        boundary/           # AD-24: named pipe / unix socket - identity read off the peer credential. NOT YET BUILT.
      stream/               # AD-22: enqueue-and-return publisher, bounded buffer, drops slow subscribers. NOT YET BUILT.
  ...
  clients/                  # AD-17: OUTSIDE the distribution - nothing in src/ imports from here
    static-compare/         # today this is scripts/compare.py + scripts/render_run.py; the move is pending
    tree/                   # F19 reference experience - BUILT: render_tree.py, recorded runs only
  cases/
    calibration/
...
| ~~F14 Side-by-Side View~~ | superseded by F17–F19 | AD-17 |
| F15 Gateway Metering | `adapters/metering` | AD-10, AD-13 |
| F16 Submission Artifact | consumes `harness` + record store | AD-9, AD-10, AD-17 |
| F17 Decision Stream Port | `ports/stream` + `adapters/stream` | AD-17, AD-22, AD-23, AD-20 |
| F18 Approval Port | `ports/approval` + `adapters/approval/boundary` | AD-12, AD-17, AD-23, AD-24, AD-20 |
| F19 Reference Experience | **outside the distribution** — `clients/` | AD-17 |
...
- **A networked approval boundary.** AD-24 binds the MVP to loopback. Remote clients need an authenticated principal, transport security, rate limiting and a threat model for the approval authorisation — none of which the same-host posture has to answer, and all of which peer-credential identity stops providing the moment the connection leaves the machine.
- **Approver authorization — who may satisfy which clause** (PRD Q11). AD-24 verifies *identity*; it labels the entitlement gap `authorization-unchecked`. Closing it means an approver role or principal list on the Outcome Contract, which reaches AD-7's closed loader and the freeze. Deliberately not done here.
- **Subscriber replay and backfill.** AD-22 drops a slow subscriber rather than buffering it. A client that wants the decisions it missed re-reads the sealed log afterwards; a resumable cursor is a different design and is not needed for a same-host MVP.
