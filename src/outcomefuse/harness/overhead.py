...
def save_study(study: OverheadStudy, path: Path) -> Digest:
    """Write the study out so a preregistration can cite it by content."""
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        "# GENERATED by outcomefuse.harness.overhead_study — do not hand-edit.\n"
        "# Its digest is cited by the preregistration, so editing it invalidates\n"
        "# the claim that the targets were derived from a measurement.\n"
        + yaml.safe_dump(study.model_dump(mode="json"), sort_keys=True),
        encoding="utf-8",
    )
    return study.digest()


def load_study(path: Path) -> OverheadStudy:
    try:
        raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    except FileNotFoundError as exc:
        raise OverheadRefused(f"no overhead study at {path}") from exc
    try:
        return OverheadStudy.model_validate(raw)
    except Exception as exc:
        raise OverheadRefused(f"the overhead study at {path} is malformed: {exc}") from exc
